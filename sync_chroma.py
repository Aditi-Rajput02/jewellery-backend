...
sync_chroma.py
--------------
One-time script to populate ChromaDB from MongoDB descriptions.
Run this while the server is stopped, or use POST /rag/sync after restarting.
...
from rag_pipeline import _get_collection, count_transcripts

OPENAI_API_KEY = os.environ["OPENAI_API_KEY"]
IMG_BASE = "https://bgjewels.jewelscore.com/f/shortimage/"

def main():
    print("=== ChromaDB Sync from MongoDB ===")
    client = OpenAI(api_key=OPENAI_API_KEY)
    db = get_db()

    # Collect all SKUs with descriptions
    sku_desc_map = {}
    ...
    if total_mongo == 0:
        print("Nothing to sync. Add descriptions first via /catalog/describe-from-url")
        return

    # Check existing ChromaDB entries
    col = _get_collection()
    existing_ids = set()
    try:
    ...
    print(f"Already in ChromaDB: {len(existing_ids)} | To sync: {len(to_sync)}")

    if not to_sync:
        print(f"ChromaDB is already up to date! Total: {count_transcripts()}")
        return
    ...
    print(f"\n=== Done! Synced: {synced} | Errors: {errors} | Total in ChromaDB: {total} ===")
